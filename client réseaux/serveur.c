...
void bzeroWindows(void *blocs, size_t taille) {
	memset(blocs, '\0', taille);
}

//Lit une ligne d'un "fichier" (ici un socket), le place dans le buffer passé en paramètre et renvoie la taille du message lu.
int readWindows(int socket, char* buffer, int taille_buffer) {
	return recv (socket, buffer, taille_buffer, 0);
}

//Ecrit un message dans un fichier (dans ce cas d'utilisation, envoie un message vers un socket) et renvoie la taille du message envoyé (en nombre d'octets)
int writeWindows(int socket, char* message, int taille_message) {
	return send (socket, message, taille_message , 0) ;
}
...
PGresult *effectuer_paiement (PGconn *connect , double montant , char *num_carte , char *id_terminal , char *compte_terminal , int *code_retour){
	...
	char type_transaction [] = "paiment par carte" ;
	
	char *num_compte = (char*) malloc (sizeof (char) * 9) ;
	if (num_compte == NULL){
		perror ("\nERREUR ALLOCATION MEMOIRE\n") ;
		exit (1) ;
	}
	recuperer_num_compte (num_carte , connect , code_retour , &num_compte) ;
	double solde = (double) get_solde_client (num_compte , connect , code_retour) ;
...
void generer_log (int code_retour){
	...
	char *date_ = (char*) malloc (sizeof (char) * 30) ;
